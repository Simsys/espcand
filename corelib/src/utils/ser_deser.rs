...
impl<const CAP: usize> Default for Ser<CAP> {
    fn default() -> Self {
        Self { buf: Vec::new() }
    }
}
...
impl<const CAP: usize> Serialize for Ser<CAP> {
    ...
    fn add_uint(&mut self, i: impl Into<u32>) -> Result<(), Error> {
        ...
        self.buf
            .extend_from_slice(&ibuf[idx..IBUF_LEN])
            .map_err(|_| Error::SerializeError)?;
        Ok(())
    }
...
impl<const CAP: usize> Default for DeSer<CAP> {
    fn default() -> Self {
        Self {
            vec: Vec::new(),
            head: 0,
...
mod tests {
    ...
    #[test]
    fn ok_deser_simple() {
        let mut de_ser = DeSer::<40>::default();
        ...
        let mut de_ser = DeSer::<40>::default();
        de_ser
            .extend_from_slice(b",1a2b,456,1a2b3c4d5e6f7081\n")
            .unwrap();
        ...
        assert_eq!(de_ser.is_end, false);
        assert_eq!(
            de_ser.get_slice_hex().unwrap().as_slice(),
        ...
        let mut de_ser = DeSer::<40>::default();
        de_ser
            .extend_from_slice(b",1a2x,45a,001a2b3c4d5e6f7081,1\n")
            .unwrap();
    ...
    #[test]
    fn ok_ser_simple() {
        let mut ser: Ser<40> = Ser::default();
